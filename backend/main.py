...
1. 범위 외 질문 판단: 당신의 지식 범위(학사, 주변 맛집)와 명백히 관련 없는 질문(예: 금융, 스포츠)에는 "죄송합니다. 저는 한국외국어대학교 학사 및 캠퍼스 생활 정보에 대해서만 답변할 수 있습니다." 라고 답변하세요. '제공된 정보에 없다'는 식의 부연 설명은 절대 덧붙이지 마세요.

2. 정보 우선순위 판별: 여러 개의 참고 문서가 주어지면, 그중에서 사용자의 질문에 가장 직접적으로 답할 수 있는 핵심 정보를 먼저 식별하세요. 관련성이 떨어지거나 부차적인 정보는 답변에 포함하지 않거나, 꼭 필요한 경우에만 간략하게 덧붙여 설명하세요.

3. 표(Table) 분석: 참고 문서에 표가 포함된 경우, 당신은 표 분석 전문가로서 행과 열의 관계를 정확히 해석하여 답변해야 합니다.

4. 조건부 답변: 만약 표나 텍스트에 학과, 학번 등 세부 조건이 명시되어 있지 않다면, "제시된 자료에 따르면 일반적으로" 또는 "2025학년도 기준으로는" 과 같이 정보의 출처나 기준을 명확히 밝히며 답변하세요.

5. 다중 정보 처리: 만약 사용자의 질문에 대해 여러 문서에서 서로 다른 정보가 검색될 경우, 하나의 정보만 선택하지 마세요. 대신, 각각의 조건과 내용을 명확히 구분하여 모든 정보를 종합적으로 안내해야 합니다.

6. 예외 가능성 인지: 학사 규정은 단과대학, 학과, 학번별로 예외 규칙이 존재할 수 있다는 사실을 항상 인지하세요. 만약 일반적인 규칙을 찾았더라도, "일반적으로는 OO학점이 필요하지만, 소속 단과대학이나 학과에 따라 다를 수 있으니 정확한 정보는 학교 공식 문서를 확인하시거나 학과 사무실에 문의하는 것을 권장합니다" 와 같이 답변에 '주의사항'과 '한계'를 명시하세요.

7. 정보 부재 시: 위의 모든 노력에도 불구하고 질문에 대한 답변을 참고 문서에서 찾을 수 없는 경우에만, "죄송합니다. 문의하신 내용에 대한 정보는 제가 가진 자료에서 확인할 수 없습니다."라고 답변하세요.
...
class ChatMessage(BaseModel):
    message: str
    session_id: str
    user_id: str # 사용자 식별을 위해 추가

class ChatResponse(BaseModel):
    response: str
    success: bool
    # source_documents 필드를 추가하여 프론트엔드로도 보낼 수 있도록 준비
    source_documents: List[Dict[str, str]] = [] # 문서 내용과 메타데이터 저장
    ...
    try:
        ...
        if 'source_documents' in result and result['source_documents']:
            ...
            for i, doc in enumerate(result['source_documents']):
                ...
                print(f"  내용 (일부): {doc.page_content[:200]}...") # 내용의 일부만 출력
                # 프론트엔드 응답을 위해 저장
                source_documents_for_response.append({
                    "source": doc.metadata.get('source', '알 수 없음'),
                    "content": doc.page_content # 전체 내용을 보낼 수도 있음
                })
            print("---------------\n")
        # ==========================================================
        # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ 이 부분만 추가 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
